...
public class TestBecomeAMentor {
    ...
    @Test(description = "Verifies the process of a user requesting to become a mentor, including admin approval for different browser types.", dependsOnMethods = {"org.shikshalokam.frontend.TestMenteeSignUp.testSignup"})
    public void testBecomeMentor() {
        ...
        if (PWBasePage.PWBrowser == PWBrowser.chromePixel4a || PWBasePage.PWBrowser == PWBrowser.msedgePixel4a) {
            ...
            adminRobot.openApp();
            adminRobot.sees(AppAllPages.loginPage).loginToApp(fetchProperty("defaultorgadmin.user"), fetchProperty("defaultorgadmin.password"));
